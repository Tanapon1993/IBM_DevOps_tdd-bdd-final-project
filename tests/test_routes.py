...
class TestProductRoutes(TestCase):
    ...
    def test_delete_product(self):
        ...
        # Check to make sure they are deleted
        response = self.client.get(f"{BASE_URL}/{product.id}")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_list_by_available(self):
        ...
        available_count = len([product for product in products if product.available == test_product.available])
        response = self.client.get(BASE_URL, query_string={"available": test_product.available})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.get_json()
